...
const GraphDataController: FC<{ dataset: Dataset; filters: FiltersState }> = ({
  ...
  useEffect(() => {
    ...
    dataset.edges.forEach(([source, target, edgename]) =>
    ...
    const sizenodes = graph
      .nodes()
      .map((node) => graph.getNodeAttribute(node, "sizenode"));
    const minDegree = Math.min(...sizenodes);
    const maxDegree = Math.max(...sizenodes);
    ...
    graph.forEachNode((node) =>
      graph.setNodeAttribute(
        ...
        //   MIN_NODE_SIZE
        graph.getNodeAttribute(node, "sizenode") / 2
      )
    );
